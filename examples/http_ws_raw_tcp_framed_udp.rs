//!
//! HTTP/1.1 + HTTP/2 + WebSocket + Raw TCP + Framed UDP
//! 所有协议共用8080端口
//! 运行: cargo run --example http_ws_raw_tcp_framed_udp
//!
...
fn setup_http() -> HttpRouter {
    ...
    router.get("/", |ctx: &mut Context| {
        ctx.send("{\"combo\":\"http_ws_raw_tcp_framed_udp\",\"protocols\":[\"http1\",\"http2\",\"ws\",\"raw_tcp\",\"framed_udp\"]}", None);
        true
    });
    router
}
...
async fn main() -> Result<()> {
    println!("\n=== http_ws_raw_tcp_framed_udp (port 8080) ===");
    ...
    http.get("/ws", |_ctx: &mut Context| true).middleware(setup_ws());
