...
use crate::{
    connection::context::{HttpMetadata, SharedWriter},
    http::protocol::{header::HeaderKey, status::StatusCode, version::HttpVersion},
};

pub struct Response<'a, W> {
    pub writer: &'a SharedWriter<W>,
    pub meta: &'a HttpMetadata,
}
    ...
    W: AsyncWriteExt + Unpin,
{
    ...
    pub async fn send_response(&mut self) -> anyhow::Result<()> {
        ...
        println!("Body: {:?}", String::from_utf8_lossy(&self.meta.body));

        println!("Status: {}", self.meta.status.to_str());

        // 2. 调用 send_local。注意这里 headers 传入引用，body 传入引用
        // 如果 send_local 定义需要 &mut HashMap，则传入 &mut headers
        self.send(&self.meta.headers, &self.meta.body, self.meta.status, self.meta.version)
            .await
    }
}
