...
mod tests {
    ...
    // 辅助函数：将闭包转换为 PipeManager 需要的 BoxFuture
    fn to_boxed_cb<T, F>(f: F) -> impl Fn(T) -> BoxFuture<'static, ()> + Send + Sync + 'static
    where
        T: Send + 'static,
    ...
    async fn test_pipe_register_and_send_success() {
        ...
        manager.register("add_one", move |val: usize| {
    ...
    async fn test_pipe_conflict_error() {
        ...
        // 第一次注册成功
        let res1 = manager.register("unique_pipe", |_: String| async {}.boxed()).await;
        assert!(res1.is_ok());

        // 第二次注册同名管道，应该返回 Err
        let res2 = manager.register("unique_pipe", |_: String| async {}.boxed()).await;
        assert!(res2.is_err());
    ...
    async fn test_pipe_type_mismatch() {
        ...
        // 注册为处理 String 的管道
        manager.register("string_pipe", |_: String| async {}.boxed()).await.unwrap();
...
async fn test_pipe_registration_race_condition_fixed() {
    ...
    for _ in 0..num_tasks {
        ...
        handles.push(tokio::spawn(async move {
            bar.wait().await;
            mgr.register(pipe_name, |_: String| async {}.boxed()).await
        }));
    }
