...
mod tests {
    ...
    async fn test_global_lifecycle_management() {
        ...
        globals.add_exit("tcp", tcp_token.clone(), tcp_handle.abort_handle()).await;
        ...
        globals.add_exit("udp", udp_token.clone(), udp_handle.abort_handle()).await;
        ...
        assert!(final_services.is_empty(), "Exits map should be empty after shutdown");
        ...
        assert!(!is_alive, "Child connection should be closed by parent token cancellation");
