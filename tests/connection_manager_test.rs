...
mod tests {
    ...
    async fn test_shutdown() {
        ...
        let cancellation_token: tokio_util::sync::CancellationToken = tokio_util::sync::CancellationToken::new();
        manager.add(addr, tokio::spawn(async {}).abort_handle(), cancellation_token, false, None);
    ...
    async fn test_extreme_deactivate() {
        ...
        let cancellation_token: tokio_util::sync::CancellationToken = tokio_util::sync::CancellationToken::new();
        // 注入一个连接
        manager.add(addr, tokio::spawn(async {}).abort_handle(), cancellation_token, true, None);
    ...
    async fn test_connect_success() {
        ...
        // 场景：正常连接
        let result = manager.connect::<RawCodec, RawCodec, _, _>(
            addr,
            global,
            |_ctx| async move {
                // 业务逻辑：收到连接后打印或执行简单操作
    ...
    async fn test_connect_closure_execution() {
        ...
        let _ = manager.connect::<RawCodec, RawCodec, _, _>(
            addr,
            global,
            |_ctx| async move {
                // 业务逻辑：收到连接后打印或执行简单操作
