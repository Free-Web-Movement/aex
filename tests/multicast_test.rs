...
fn test_multicast_scope_from_addr() {
    ...
    assert_eq!(MulticastScope::from_addr(&local), Some(MulticastScope::Local));
    
    let global = Ipv4Addr::new(225, 0, 0, 1);
    assert_eq!(MulticastScope::from_addr(&global), Some(MulticastScope::Global));
    
    let admin = Ipv4Addr::new(239, 0, 0, 1);
    assert_eq!(MulticastScope::from_addr(&admin), Some(MulticastScope::Admin));
    
    let site_local = Ipv4Addr::new(239, 255, 0, 1);
    assert_eq!(MulticastScope::from_addr(&site_local), Some(MulticastScope::SiteLocal));
    
    let org = Ipv4Addr::new(239, 192, 0, 1);
    assert_eq!(MulticastScope::from_addr(&org), Some(MulticastScope::Organization));
}
