...
mod aex_tests {
    ...
    async fn test_aex_server_coverage() {
        ...
        let test_result = timeout(Duration::from_secs(10), async {
            ...
            tr.on::<RawCodec, _, _>(10, |_, _, _| async move { Ok(true) });

            // 4. 注册 UDP 路由 (ID 20)
            let mut ur = UdpRouter::new();
            ur.on::<RawCodec, _, _>(20, |_, addr, socket| async move {
            ...
            udp_client.send_to(&rawudp, actual_addr).await.unwrap(); // 发送 ID 20
